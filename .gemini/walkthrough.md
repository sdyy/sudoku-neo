...
*   當玩家點選網格中某個已有數字的格子（例如點選 `5`）時，不僅所有已填入 `5` 的格子會高亮成金色。
*   其他**所有未填格子的草稿（Pencil Marks）中，包含 `5` 的候選數也會同步亮起**！
*   **深色模式下**：草稿中匹配的數字會變成高飽和的**金黃色 (`#fbbf24`)**、背景帶有半透明金色光暈，且微幅放大 `1.15 倍`。
*   **淺色模式下**：草稿中匹配的數字會變成**深琥珀色 (`#b45309`)**，與淺色盤面高亮完美契合。
